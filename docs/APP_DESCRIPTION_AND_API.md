...
- **Можно ли по API взаимодействовать с этим приложением?**  
  Да. При запуске через uvicorn (или Docker) доступен **Admin API** (FastAPI) на порту 8000. Через него можно получать и менять настройки Telegram и LLM, проверять соединения, перезапускать бота. Для доставки сообщений пользователям или вызова LLM используется только сценарий «пользователь пишет в Telegram → бот → LLM → ответ в чат»; отдельного публичного API для отправки сообщений или чата с LLM в приложении нет.
