...
Пункты **1–4** — **ядро** приложения (без них нет ни чата, ни вызова инструментов). Всё остальное — **инструментарий** (встроенная инфраструктура) и **надстройки** (плагины).
...
└─────────────────────────────────────────────────────────────────────────────────┘
                                        ...
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  ПЛАГИНЫ (надстройки) — подключаемые модули с инструментами                      │
│  Живут в plugins/; описываются в plugin.yaml; код в handlers.py.                  │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│   Встроенные (built-in):                                                         │
│   ├── plugins/builtin/calculator/     → инструмент calculate                     │
│   └── plugins/builtin/datetime_tools/ → инструменты get_datetime, get_weekday     │
│                                                                                  │
│   Бизнес-плагины:                                                                │
│   └── plugins/worklog_checker/        → инструмент get_worklogs (Jira/Tempo)      │
...
| **Плагины** | builtin (calculator, datetime_tools), worklog_checker, будущие (hr_service, reminder, …) | Конкретные инструменты и бизнес-логика; надстраиваются поверх ядра и инструментария |
