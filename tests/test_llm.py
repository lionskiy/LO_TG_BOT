...
async def test_get_reply_returns_content():
    with patch.object(llm, "get_active_llm", return_value=("openai", "gpt-4o-mini", {"api_key": "sk-test"})):
        mock_openai = AsyncMock(return_value=("Hello from model", None))
        with patch.dict(llm._HANDLERS, {"openai": mock_openai}):
            content, tool_calls = await llm.get_reply([{"role": "user", "content": "Hi"}])
    assert content == "Hello from model"
...
async def test_get_reply_strips_whitespace():
    with patch.object(llm, "get_active_llm", return_value=("openai", "gpt-4o-mini", {"api_key": "sk-test"})):
        with patch.dict(llm._HANDLERS, {"openai": AsyncMock(return_value=("answer", None))}):
            content, _ = await llm.get_reply([{"role": "user", "content": "Hi"}])
...
async def test_get_reply_empty_content_returns_empty_string():
    with patch.object(llm, "get_active_llm", return_value=("openai", "gpt-4o-mini", {"api_key": "sk-test"})):
        with patch.dict(llm._HANDLERS, {"openai": AsyncMock(return_value=(None, None))}):
            content, _ = await llm.get_reply([{"role": "user", "content": "Hi"}])
...
async def test_get_reply_uses_active_provider_and_model():
    with patch.object(
        ...
        mock_groq = AsyncMock(return_value=("Groq reply", None))
        with patch.dict(llm._HANDLERS, {"groq": mock_groq}):
            content, _ = await llm.get_reply([{"role": "user", "content": "Hi"}])
    assert content == "Groq reply"
